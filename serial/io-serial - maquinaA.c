...
//Alterações Carlos 30-10-2017 implementação inicial
//Variáveis 
int ch, tam;
char c[200], d[15]=">/dev/ttyS0", f[20000];
//Laço infinito (até agora...)
while(1){
	//Pede o comando para o usuário
	printf("Declare o comando \n");
	scanf("\n %s", c);
	//Pega a tecla digitada no teclado
	ch=getchar();
	//Se digitar Enter
	if(ch==10){
		//Concatena o comando com o redirecionamento para a porta serial
		strcat(c,d);
		//Pega o tamanho do que será escrito/transmitido
		tam = sizeof(c);
		//Manda para a porta serial para a máquina B
		write(fd, &c, tam);
	}
	//Se porta serial com conteúdo
	if(read(fd, &f, sizeof(f)) >= 0){
		//Não sei se temos que colocar em uma string ou o que
		//write(kbd, &f, sizeof(f)); 
		//ou teriamos que executar um echo?
	}
